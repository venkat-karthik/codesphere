...
export default function ActiveTestPage() {
    ...
    const handleRealSubmit = async () => {
        ...
        try {
            ...
            toast.error("Failed to submit test. Please check your connection.");
        } finally {
                        ...
                        {testResult?.achievement && (
                            <div className="mt-4 pt-4 border-t border-border/40">
                                ...
                                <div className="text-foreground font-semibold">
                                    {testResult.achievement.title}
                                </div>
                                <p className="text-[10px] text-muted-foreground mt-1 tracking-tight">Visit your profile to see your new badge</p>
                            </div>
                        )}
                    </div>
        ...
        <div className="min-h-screen bg-black flex flex-col">
            ...
            <div className="h-16 border-b border-border/40 bg-card/30 flex items-center justify-between px-6 sticky top-0 z-50">
                <div className="flex items-center gap-4">
                    <div className="w-8 h-8 rounded-full bg-primary/20 flex items-center justify-center text-primary font-bold text-xs ring-1 ring-primary/40">
                        C
                    </div>
                    <div>
                        <div className="font-semibold text-sm">React Hooks Masterclass</div>
            ...
            <div className="flex-1 flex max-w-6xl w-full mx-auto p-6 gap-8">
                ...
                <div className="flex-1 max-w-3xl">
                    <div className="flex items-center justify-between mb-8">
                        <h3 className="text-lg font-medium text-muted-foreground">Question {currentIdx + 1} of {MOCK_QUESTIONS.length}</h3>
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`p-4 rounded-xl border-2 transition-all cursor-pointer flex items-center gap-4 ${isSelected
                                                ...
                                                : "border-border/40 hover:border-primary/50 bg-card/20"
                                                }`}
                                        >
                    ...
                    <div className="flex items-center justify-between border-t border-border/40 py-6">
                        ...
                        {currentIdx < MOCK_QUESTIONS.length - 1 ? (
                            ...
                            >
                                Finish & Submit <AlertCircle className="w-4 h-4" />
                            </Button>
                        )}
                    </div>
                </div>
                        ...
                        <CardContent className="p-6">
                                ...
                                {MOCK_QUESTIONS.map((_q, index) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`w-10 h-10 rounded-lg flex items-center justify-center text-sm font-medium transition-colors ${isCurrent ? "bg-primary text-primary-foreground ring-2 ring-primary ring-offset-2 ring-offset-background" :
                            ...
                            <div className="space-y-3 text-sm text-muted-foreground border-t border-border/40 pt-6">
                                <div className="flex items-center gap-3">
                                    <div className="w-4 h-4 rounded-md bg-primary/20 border border-primary/30" />
                                    <span>Answered</span>
                                </div>
                                <div className="flex items-center gap-3">
                                ...
                                <div className="flex items-center gap-3">
                                    <div className="w-4 h-4 rounded-md bg-primary text-primary-foreground flex items-center justify-center font-bold" />
                                    <span>Current</span>
                                </div>
                            </div>
                        </CardContent>
