      ...
      <Link href={item.path}>
        ...
        >
          <div className="flex items-center gap-3">
            <div className={`p-1.5 rounded-lg transition-colors ${isActive ? 'bg-white/20' : 'bg-sidebar-foreground/10 group-hover:bg-sidebar-foreground/20'}`}>
  ...
  if (!isAuthenticated) {
    return (
      <motion.div 
        ...
        className="fixed left-0 top-0 h-full border-r z-50 w-72 flex flex-col bg-sidebar-background"
      >
        <div className="p-8">
          <Link href="/">
        ...
        <div className="p-6 h-full flex flex-col overflow-y-auto scrollbar-hide">
          <div className="flex items-center justify-between mb-10">
            <Link href="/">
              <div className="flex items-center space-x-3 cursor-pointer group">
                ...
                <h1 className="text-xl font-black tracking-tighter text-sidebar-foreground overflow-hidden">CodeSphere</h1>
              </div>
            </Link>
          ...
          <div className="space-y-8 flex-1">
            {sections.map((section, idx) => (
              <div key={idx}>
                <h3 className="px-4 text-[10px] font-black uppercase tracking-widest text-sidebar-foreground/50 mb-3">{section.title}</h3>
            ...
            {(isAdmin || isSubAdmin) && (
              <div>
                <h3 className="px-4 text-[10px] font-black uppercase tracking-widest text-sidebar-primary/70 mb-3">Administration</h3>
                <div className="space-y-1">
                  <NavItem item={{ id: 'platform-analytics' as Section, icon: BarChart3, label: 'Analytics', path: '/admin/analytics' }} />
                  <NavItem item={{ id: 'admin-live-classes' as Section, icon: Video, label: 'Live Classes', path: '/admin/live-classes' }} />
                  <NavItem item={{ id: 'admin-pdf-resources' as Section, icon: FileText, label: 'PDF Resources', path: '/admin/resources' }} />
                  <NavItem item={{ id: 'admin-video-resources' as Section, icon: Play, label: 'Video Resources', path: '/admin/videos' }} />
                  <NavItem item={{ id: 'admin-sphere-map' as Section, icon: Globe2, label: 'Sphere Map', path: '/admin/sphere-map' }} />
                </div>
              </div>
            )}
          </div>
          ...
          <div className="mt-10 pt-6 border-t border-sidebar-border">
            <div className="flex items-center justify-between px-2">
              <ThemeToggle />
              <Button variant="ghost" size="icon" onClick={() => logout()} className="text-sidebar-foreground/60 hover:text-destructive">
